...
function NudgeBanner({ switched, onSwitch, onProceed }) {
  if (switched) {
    return (
      <div className="flex items-start gap-4 rounded-2xl border border-emerald-200 dark:border-emerald-500/20 bg-gradient-to-r from-emerald-50/50 to-green-100/30 dark:from-emerald-950/30 dark:to-green-950/20 p-6 shadow-sm dark:shadow-lg dark:shadow-black/20 transition-all duration-500 animate-scale-in">
        ...
        <div>
          <p className="text-sm font-extrabold text-slate-900 dark:text-white">Optimal Fit Selected ✨</p>
  ...
  return (
    <div className="rounded-2xl border border-amber-200 dark:border-amber-500/20 p-6 shadow-sm dark:shadow-lg dark:shadow-black/20 bg-gradient-to-r from-amber-50/50 to-orange-100/30 dark:from-amber-950/30 dark:to-orange-950/15 transition-all duration-500 animate-scale-in">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-extrabold text-slate-900 dark:text-white">⚠️ Heads up about your fit</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2.5">
            ...
            <button
              onClick={onProceed}
              className="cursor-pointer rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/40 hover:bg-slate-200 dark:hover:bg-slate-800 hover:border-slate-350 dark:hover:border-white/20 text-xs font-bold text-slate-700 dark:text-slate-200 px-5 py-2.5 shadow-sm transition-all duration-300"
            >
              Keep UK8
            </button>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 animate-fade-in-up">
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-6">
          {['Amazon', 'Sports', 'Footwear', 'Nike Air Max'].map((crumb, i, arr) => (
            <span key={crumb} className="flex items-center gap-1.5">
              <span className={`hover:text-orange-600 dark:hover:text-orange-500 cursor-pointer transition-colors ${i === arr.length - 1 ? 'text-slate-800 dark:text-slate-300' : ''}`}>
                {crumb}
              </span>
              {i < arr.length - 1 && (
                <svg className="h-3 w-3 text-slate-300 dark:text-slate-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="flex flex-col md:flex-row gap-12">
          ...
          <div className="flex-shrink-0 w-full md:w-96 animate-fade-in-up">
            <div className="w-full aspect-square rounded-3xl bg-gradient-to-br from-slate-50 to-slate-100/50 dark:from-slate-900/40 dark:to-slate-950/20 border border-slate-200 dark:border-white/5 shadow-md dark:shadow-2xl shadow-black/[0.02] dark:shadow-black/30
                            flex flex-col items-center justify-center gap-3 relative overflow-hidden group">
              {/* Corner badge */}
              <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-200 dark:border-white/5 px-3 py-1.5 rounded-full text-[9px] font-extrabold text-slate-600 dark:text-slate-350 shadow-sm z-10">
            ...
            <div className="mt-4 flex gap-3">
              {[0, 1, 2].map(i => (
                <button
                  ...
                  className={`h-20 w-20 rounded-xl bg-slate-100 dark:bg-slate-900/40 border-2 cursor-pointer transition-all duration-300 overflow-hidden flex items-center justify-center
                    ${activeThumb === i ? 'border-orange-500 shadow-md shadow-orange-500/15 scale-105' : 'border-slate-200 dark:border-white/5 hover:border-slate-350 dark:hover:border-white/20'}`}
                >
                  {i === 0 && (
          ...
          <div className="flex-1 flex flex-col gap-6 animate-slide-in-right">
            ...
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white leading-tight tracking-tight mt-1">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-3 mt-1">
                ...
                <span className="text-sm text-slate-400 dark:text-slate-555 line-through font-medium">₹7,999</span>
                <span className="text-xs font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-100/50 dark:bg-emerald-950/40 border border-emerald-200/50 dark:border-emerald-500/25 px-2.5 py-0.5 rounded-md">
                  38% SAVED
                </span>
              </div>
            ...
            <div>
                    ...
                    <button
                      ...
                      className={`relative cursor-pointer rounded-xl border-2 px-5 py-2.5 text-xs font-bold transition-all duration-300
                        ${isSelected
                          ...
                          : 'border-slate-200 dark:border-white/10 text-slate-605 dark:text-slate-350 bg-slate-100 dark:bg-slate-900/40 hover:border-slate-350 dark:hover:border-white/25 hover:text-slate-900 dark:hover:text-white'
                        }`}
                      aria-pressed={isSelected}
                    >
                      {size}
                      {isRecommended && !isSelected && (
                        <span className="absolute -top-2 -right-1 text-[8px] font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/80 px-1.5 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-500/20 leading-none">
                          REC
                        </span>
                      )}
                    </button>
              ...
              <p className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline cursor-pointer mt-3 flex items-center gap-1">
                Check sizing guide
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md">
              ...
              >
               ...
               <button
                className="cursor-pointer flex-1 rounded-xl border-2 border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/40 py-4 text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 hover:border-slate-350 dark:hover:border-white/20 transition-all duration-300 shadow-sm"
              >
                Buy Now
              </button>
            </div>
            ...
            {/* Delivery info */}
            <div className="flex gap-6 pt-3 border-t border-slate-200 dark:border-white/5">
              <div className="flex items-center gap-2.5">
                <div className="h-8 w-8 rounded-lg bg-blue-100/50 dark:bg-blue-950/40 border border-blue-200/50 dark:border-blue-500/20 flex items-center justify-center shadow-md">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Thu, 19 June</p>
                </div>
              </div>
              <div className="flex items-center gap-2.5">
                <div className="h-8 w-8 rounded-lg bg-emerald-100/50 dark:bg-emerald-950/40 border border-emerald-200/50 dark:border-emerald-500/20 flex items-center justify-center shadow-md">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-700 dark:text-slate-300">30-day circular</p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-14 rounded-3xl bg-gradient-to-r from-blue-50/50 to-indigo-100/30 dark:from-blue-950/20 dark:to-indigo-950/10 border border-blue-200/50 dark:border-blue-500/15 p-7 flex gap-5 shadow-sm dark:shadow-lg dark:shadow-black/5 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
          ...
          <div>
            <p className="text-sm font-extrabold text-slate-900 dark:text-white mb-1.5">What is size fit prediction?</p>
