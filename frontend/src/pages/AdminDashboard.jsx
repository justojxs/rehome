...
function CircularProgress({ value, size = 48, strokeWidth = 4, color = '#FF9900' }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="transform -rotate-90">
      <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="currentColor" className="text-slate-800" strokeWidth={strokeWidth} />
...
function SummaryCard({ label, value, colorClass, icon, total }) {
  ...
  return (
    <div className={`${c.bg} rounded-2xl border border-white/5 shadow-sm px-6 py-5 flex items-center justify-between
                     transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-default group`}>
      <div className="flex flex-col gap-1.5">
        <span className="text-[10px] font-extrabold text-slate-450 uppercase tracking-widest">{label}</span>
        <span className="text-3xl font-black text-white tracking-tight animate-count-up">{value}</span>
      </div>
      <div className="relative h-12 w-12 flex items-center justify-center">
        <CircularProgress value={pct || (value > 0 ? 100 : 0)} size={48} color={c.ring} />
        <div className="absolute inset-0 flex items-center justify-center text-slate-450 group-hover:scale-110 transition-transform duration-300">
          {icon}
        </div>
      </div>
    </div>
    ...
    <tr className="animate-pulse">
      {Array.from({ length: 8 }).map((_, i) => (
        <td key={i} className="px-5 py-5">
          <div className="h-4 bg-slate-800/60 rounded-lg w-full shimmer-bg" />
        </td>
      ))}
    </tr>
  )
    ...
    <div className="min-h-screen py-10 mesh-gradient">
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4 animate-fade-in-up">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900 border border-white/5 text-orange-400 text-[10px] font-bold tracking-widest mb-3">
              <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" />
              </svg>
              OPERATIONS CONTROL CENTER
            </div>
            <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight">Admin Dashboard</h1>
          ...
          <button
            ...
            className="self-start sm:self-center inline-flex items-center gap-2.5 rounded-xl bg-slate-900/50 border border-white/10 px-5 py-2.5 text-xs font-bold text-slate-200 hover:bg-slate-800 hover:border-white/20 cursor-pointer shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md"
          >
          ...
          <div className="overflow-x-auto">
                ...
                {loading
                  ...
                  : returns.map((row, index) => {
                      ...
                      return (
                        <tr
                          key={row.id}
                          className="table-row-stagger hover:bg-white/[0.02] transition-colors duration-200"
                          style={{ '--stagger-delay': `${index * 80}ms` }}
                        >
                          {/* Order ID */}
                          <td className="px-5 py-5 whitespace-nowrap">
                            <span className="font-mono text-xs font-bold text-slate-300 bg-slate-900/60 border border-white/5 px-2.5 py-1 rounded-md">{row.order_id}</span>
                          </td>

                          {/* Product Name */}
                          <td className="px-5 py-5 text-slate-100 font-bold max-w-[200px]">
                            <span className="line-clamp-2 leading-relaxed" title={row.product_name}>{row.product_name}</span>
                          </td>

                          {/* Return Reason */}
                          <td className="px-5 py-5 whitespace-nowrap">
                            <span className="text-xs font-bold text-slate-350 bg-slate-900/60 border border-white/5 px-2.5 py-1 rounded-full">{row.return_reason}</span>
                          </td>

                          {/* Customer Condition */}
                          <td className="px-5 py-5 text-slate-300 font-semibold whitespace-nowrap">
                          ...
                          <td className="px-5 py-5 whitespace-nowrap">
                            <div className="flex flex-col gap-2.5 min-w-[140px]">
                              ...
                              <button
                                ...
                                className="cursor-pointer inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-slate-900/40 px-4 py-2.5 text-[11px] font-bold text-slate-200 hover:bg-slate-800 hover:border-white/20 transition-all shadow-sm whitespace-nowrap"
                              >
                                <svg className="h-3.5 w-3.5 text-slate-550" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {!loading && returns.length === 0 && (
              <div className="py-24 text-center">
                <div className="h-16 w-16 mx-auto rounded-full bg-slate-900/50 flex items-center justify-center border border-white/10 mb-4 shadow-lg shadow-black/10">
                  <svg className="h-8 w-8 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                  </svg>
                </div>
                <p className="text-sm font-bold text-slate-350">No returns found</p>
